import {compare, p3dManhattan, parseData, Scanner} from "./day19";
...
test('parses', () => {
    const scanners = parseData(testData.split('\n'))

    expect(scanners).toHaveLength(5)
    expect(scanners[0].blips[0].x).toBe(404)
    expect(scanners[0].blips[0].y).toBe(-588)
    expect(scanners[0].blips[0].z).toBe(-901)
    expect(scanners[4].id).toBe("4")
    expect(scanners[4].blips[5].x).toBe(-660)
    expect(scanners[4].blips[5].y).toBe(-479)
    expect(scanners[4].blips[5].z).toBe(-426)
})
...
test('compares', () => {
    ...
    expect(unPositionedScanners).toHaveLength(0)
...
test('manhatten', ()=>{
    expect(p3dManhattan({x:1105, y:-1205, z:1229}, {x:-92, y:-2380, z:-20})).toBe(3621)
})